...
def extract_records(pdf_path):
    ...
    for page_num in range(len(doc)):
        page = doc[page_num]
        # PyMuPDF handles rotation automatically
        text = page.get_text("text")
        
        # Find lines with NF pattern
        lines = text.split('\n')
        for i, line in enumerate(lines):
            # NF Nº 00059523 or NF 59523 or NF. 000000053378 a 00053378 or NFE 000000852
            ...
            # Valor is in the next 1-3 lines (histórico may span multiple lines)
            credito = 0.0
            for offset in range(1, 4):
                if i + offset < len(lines):
                    value_match = re.search(r'R\$\s*([\d.,]+)', lines[i+offset])
                    if value_match:
                        credito = parse_value(value_match.group(1))
                        break
            
            if credito == 0.0 and not data_emissao:
                continue
    ...
    # Sum duplicate NFs (same NF number = same invoice, sum the credits)
    summed = {}
    for r in records:
        key = r["num_nf"]

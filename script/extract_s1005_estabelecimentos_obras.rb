...
options = {
	output_dir: Rails.root.join("tmp", "estabelecimentos_s1005"),
	current_on: Date.today
}

parser = OptionParser.new do |opts|
	opts.banner = "Uso: ruby script/extract_s1005_estabelecimentos_obras.rb [opcoes] ARQUIVO_OU_PASTA..."
...
result = Esocial::EstabelecimentosObrasExtractor.call(
	source_paths: ARGV,
	output_dir: options[:output_dir],
	current_on: options[:current_on]
)

puts "Eventos S-1005 encontrados: #{result.rows.size}"
puts "Estabelecimentos/obras no quadro: #{result.current_rows.size}"
...
if result.rows.empty?
	warn "Nenhum evento evtTabEstab/S-1005 foi encontrado nas fontes informadas."
	exit 2
end
